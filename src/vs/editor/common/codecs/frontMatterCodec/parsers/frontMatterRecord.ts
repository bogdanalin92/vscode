...
export class FrontMatterRecord extends FrontMatterToken {
	...
	public static fromTokens(
		...
		assert(
			firstToken instanceof FrontMatterRecordName,
			`First token must be a front matter record name, got '${firstToken}'.`,
		);
		assert(
			secondToken instanceof FrontMatterRecordDelimiter,
			`Second token must be a front matter record delimiter, got '${secondToken}'.`,
		);
		assert(
			thirdToken instanceof FrontMatterValueToken,
			`Third token must be a front matter value, got '${thirdToken}'.`,
		);
